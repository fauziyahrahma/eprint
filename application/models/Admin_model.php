...
class Admin_model extends CI_model
{
    ...
    public function getBytgl($a, $b)
    {
        /* $this->db->select('pemesanan.*,user.username,kategori.nama_kategori,pembayaran.nama_pemesan,pembayaran.jenis_bayar,pembayaran.bukti_bayar,pembayaran.bukti_dp, SUM(jumlah_barang) jmlBarang,SUM(total_bayar) total_bayar');
        $this->db->from('pembayaran');
        $this->db->join('pemesanan', 'pembayaran.id_bayar = pemesanan.id_bayar');
        $this->db->join('kategori', 'pemesanan.id_kategori = kategori.id_kategori');
        $this->db->join('user', 'user.id_user = pemesanan.id_user');
        $this->db->where('pemesanan.id_bayar !=', 0);
        $this->db->where('pemesanan.tanggal_checkout >=', $a);
        $this->db->where('pemesanan.tanggal_checkout <=', $b); */
        //$this->db->group_by('pemesanan.id_pesanan');

        $q = $this->db->query("SELECT pemesanan.*,user.username,kategori.nama_kategori,pembayaran.nama_pemesan, jumlah_barang as jmlBarang, total_bayar as total_bayar 
        FROM pembayaran
        ...
        WHERE pemesanan.id_bayar != 0 AND pemesanan.tanggal_checkout BETWEEN '$a' AND '$b'
        ");
        //return $this->db->get()->result_array();
